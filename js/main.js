...
(function ($) {
	...
	// Parallax handling - only initialize if needed
	...
	// Scrollax - only initialize if needed
	if ($('[data-scrollax-parent]').length > 0) {
		$.Scrollax();
	}
	...
	var scrollWindow = function () {
		...
		$(window).scroll(function () {
			...
			if (st > 350) {
				...
				if (sd.length > 0 && !sd.hasClass('sleep')) {
					sd.addClass('sleep');
				}
			}
			if (st < 350) {
	...
	var OnePageNav = function () {
		$(".smoothscroll[href^='#'], #ftco-nav ul li a[href^='#']").on('click', function (e) {
			...
			if (!$('#ftco-navbar').hasClass('scrolled')) {
				navHeight -= 20;
			}
	...
	// Custom ScrollSpy
	var customScrollSpy = function () {
		$(window).on('scroll', function () {
			var scrollPos = $(document).scrollTop();
			var navHeight = $('#ftco-navbar').outerHeight() || 0;

			// Add a small offset so it activates slightly before reaching the very top
			...
			$('#ftco-nav .nav-link').each(function () {
				...
				if (href) {
					...
					if (hashIndex !== -1) {
						...
						var $home = $('#home');
						if ($home.length > 0) {
							var homeTop = $home.offset().top - offset;
